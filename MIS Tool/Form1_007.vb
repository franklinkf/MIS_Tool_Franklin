...
Public Class Form1
    ...
    Private Sub option601()   'eNMGB Migration - Create Branch Data
        ...
        If Directory.Exists("D:\DUMP\BR_" & solid) Then
            ...
            Dim sw1 As StreamWriter = New StreamWriter("d:\dump\script\importten_FUF_USERS.bat")
        ...
        Dim sw As StreamWriter = New StreamWriter("d:\dump\script\droptenFUF_MAIN_USER.sql")
        ...
        Dim sw2 As StreamWriter = New StreamWriter("d:\dump\script\createtable.bat")
        sw2.WriteLine("@echo off")
        sw2.WriteLine("sqlplus " & import_user & "/" & import_user & "@ten @d:\dump\script\table_script.sql /nolog ")
        sw2.Close()

        processmessage("Executing Script for moving data")
        Dim sw3 As StreamWriter = New StreamWriter("d:\dump\script\movedata.bat")
        sw3.WriteLine("@echo off")
        sw3.WriteLine("sqlplus " & import_user & "/" & import_user & "@ten @d:\dump\script\Script_tomove_data.sql /nolog")
        'sw3.WriteLine("pause")
        'sw3.WriteLine(":exit;")
        sw3.Close()
        ...
        Dim sw14 As StreamWriter = New StreamWriter("d:\dump\script\GRANT_script.sql")
        ...
        Dim sw5 As StreamWriter = New StreamWriter("d:\dump\script\SYNONYM_cbs.sql")
        ...
        sw5.WriteLine("CREATE PUBLIC SYNONYM LSP FOR LSP;")
        sw5.WriteLine("GRANT SELECT, INSERT, UPDATE, DELETE ON LSP TO " & import_user & ";")
        sw5.WriteLine(" END;")
        sw5.WriteLine("/")
        sw5.WriteLine("exit;")
        sw5.Close()

        Dim sw114 As StreamWriter = New StreamWriter("d:\dump\script\GRANT_MIG.bat")
        sw114.WriteLine("@echo off")
        sw114.WriteLine("sqlplus mig/mig@ten @d:\dump\script\grant_script.sql /nolog")
        sw114.WriteLine("sqlplus cbs/cbs@ten @d:\dump\script\SYNONYM_cbs.sql /nolog")
        sw114.WriteLine(" END;")
        sw114.WriteLine("/")
        sw114.WriteLine("exit;")
        sw114.Close()

        processmessage("Executing Packages")
        Dim sw15 As StreamWriter = New StreamWriter("d:\dump\script\package.bat")
        sw15.WriteLine("@echo off")
        sw15.WriteLine("sqlplus " & import_user & "/" & import_user & "@ten @d:\dump\script\Package_script.sql /nolog")
        sw15.WriteLine("exit;")
        sw15.Close()

        Dim sw6 As StreamWriter = New StreamWriter("d:\dump\script\UPDATE_USER.bat")
        sw6.WriteLine("@echo off")
        sw6.WriteLine("sqlplus " & import_user & "/" & import_user & "@ten @d:\dump\script\Update_script.sql")
        sw6.WriteLine("pause")
        sw6.Close()
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\droptenFUF_USERS.bat")
            While flag = 1
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\createtable.bat")
            While flag = 1
                'Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\GRANT_MIG.bat")
            While flag = 1
                ' Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\importten_FUF_USERS.bat")
            While flag = 1
                ' Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\movedata.bat")
            While flag = 1
                'Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\package.bat")
            While flag = 1
                'Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
        ...
        'call package  and grant 2 times
        ...
        If flag = 0 Then
            flag = 1
            Process.Start("d:\dump\script\UPDATE_USER.bat")
            While flag = 1
                'Timer1_Tick(Nothing, Nothing)
                WorkingProcess()
            End While
    ...
    Private Sub option604()   'eNMGB Migration - Assign CustID and Account No
        ...
        If username.Length <> 6 Then
            ...
            Exit Sub
        End If
        Dim uploadslno As String = InputBox("Enter Upload SL No", "", "")
        If uploadslno = "" Then
            MsgBox("Enter upload serialno from upload register")
            Exit Sub
        End If
        If IsNumeric(uploadslno) = False Then
            MsgBox("Enter valid upload serialno from register")
            Exit Sub
        End If
        If uploadslno.Length > 3 Then
            MsgBox("Enter valid upload serialno from register")
            Exit Sub
        End If
        ...
        For Each dir In dirs
            ...
            sql = "PKGFUF_FUNCTIONS.UPDATE_CID_FILE_DATA"
            Dim cmd5 As New OracleCommand(sql, oracle_conn)
            cmd5.CommandType = CommandType.StoredProcedure
            cmd5.Parameters.Add("NEXTSLNO", OracleDbType.Int16, 10, Nothing, ParameterDirection.Input).Value = uploadslno
            cmd5.ExecuteNonQuery()

            sql = "SELECT COUNT(1) ABCD FROM C_MISPRINT"
    ...
    Private Sub option605()   'eNMGB Migration - FUF Generation
        ...
        Dim migdate As Date = InputBox("Enter Date of Migration (DD/MM/YYYY)", "Enter Value", txtdate.Text)
        'Dim processflag As String = InputBox("Process Data", "Enter Value", "Y")
        ...
        Dim folderpath1 As String = "c:\du\" & branchcode & " " & branchname & "\" & rptoption & "_" & branchcode & "_" & txtmenu.Text.ToString.Substring(18, procedure_len - 18) & "\" & "MIS Team"
        Dim folderpath2 As String = "c:\du\" & branchcode & " " & branchname & "\" & rptoption & "_" & branchcode & "_" & txtmenu.Text.ToString.Substring(18, procedure_len - 18) & "\" & "Wipro"

        Dim sql6 As String
        If IsNumeric(reportid.Substring(0, 1)) = True Then

            Dim startid As String = reportid.Substring(0, 2)
            Dim endid As String = reportid.Substring(2, 2)
        ...
        While dr6.Read
            ...
            If Directory.Exists(folderpath1) Then
                For Each file1 As String In Directory.GetFiles(folderpath1)
                    Dim filepath() As String = file1.Split("\")
                    Dim filename As String = filepath(filepath.Length - 1)
                    Dim file4 As String
                    ...
                    If filename.Contains(".") Then
                        ...
                        file4 = filename.Remove(filename.IndexOf("."), filename.Length - filename.IndexOf("."))
                        file4 = file4.Substring(5, file4.Length - 5)
                        If file4.Substring(file4.Length - 2, 1) = "_" Then
                            file4 = file4.Substring(0, file4.Length - 2)
                        Else
                            file4 = file4
                        End If
                        'MsgBox(filename)
                    ...
                    If reportid = "ALL" Then
                        If File.Exists(folderpath1 & "\" & filename) Then
                            File.Delete(folderpath1 & "\" & filename)
                        End If
                    Else
                        'If Path.GetExtension(folderpath1 & "\" & filename) = ".gz" Then
                        '    File.Delete(folderpath1 & "\" & filename)
                        'End If
                        'file4 = filename.Substring(5, filename.Length - 5)
                        'file4 = file4.Substring(0, file4.Length - 4)
                        If file4 = reportid1 Then
                            File.Delete(folderpath1 & "\" & filename)
                        End If
                    End If
                Next
            ...
            If Directory.Exists(folderpath2) Then
                For Each file1 As String In Directory.GetFiles(folderpath2)
                    Dim filepath() As String = file1.Split("\")
                    Dim filename As String = filepath(filepath.Length - 1)
                    Dim file4 As String
                    If filename.Contains(".") Then
                    ...
                    End If
                    If reportid = "ALL" Then
                        If File.Exists(folderpath2 & "\" & filename) Then
                            File.Delete(folderpath2 & "\" & filename)
                        End If
                    Else
                        'If Path.GetExtension(folderpath2 & "\" & filename) = ".gz" Then
                        '    File.Delete(folderpath2 & "\" & filename)
                        'End If
                        'file4 = filename.Substring(5, filename.Length - 5)
                        'file4 = file4.Substring(0, file4.Length - 4)
        ...
        '' CREATIING FILES IN "MIS Team" FOLDER
        Dim sql5 As String
        If IsNumeric(reportid.Substring(0, 1)) = True Then

            Dim startid As String = reportid.Substring(0, 2)
            Dim endid As String = reportid.Substring(2, 2)
            sql5 = "SELECT SLNO,REPORT_ID FROM FI WHERE REPORT_TYPE = 'FUF' AND SLNO BETWEEN " & startid & " AND " & endid & " AND REPORT_ID IN ('CU1','CU2','CU3','CU5','CU6','CU7','RCT','CU8','AC1','AC1_NND','AC3','TAM_RD','TAM_FD','TAM_KVD','LAM','LRS','AC2','AC4','ALT','SRM','CBS','SPT','CIU','DHT')  ORDER BY SLNO"

        ElseIf reportid = "ALL" Then

            sql5 = "SELECT SLNO,REPORT_ID FROM FI WHERE REPORT_TYPE = 'FUF' AND REPORT_ID IN ('CU1','CU2','CU3','CU5','CU6','CU7','RCT','CU8','AC1','AC1_NND','AC3','TAM_RD','TAM_FD','TAM_KVD','LAM','LRS','AC2','AC4','ALT','SRM','CBS','SPT','CIU','DHT') ORDER BY SLNO"

        Else

            sql5 = "SELECT SLNO,REPORT_ID FROM FI WHERE REPORT_TYPE = 'FUF' AND REPORT_ID =  '" & reportid & "' AND REPORT_ID IN ('CU1','CU2','CU3','CU5','CU6','CU7','RCT','CU8','AC1','AC1_NND','AC3','TAM_RD','TAM_FD','TAM_KVD','LAM','LRS','AC2','AC4','ALT','SRM','CBS','SPT','CIU','DHT')"

        End If

        Dim sw2 As StreamWriter
        sw2 = New StreamWriter(folderpath1 & "\Menu_Report.txt")
        Dim linedata As String
        Dim pagewidth As Integer = 120
        Dim reportslno As Integer = 0
        Dim padlength As Integer

        linedata = "KERALA GRAMIN BANK"
        padlength = ((pagewidth - linedata.Length) / 2) + linedata.Length
        linedata = linedata.PadLeft(padlength, " ")
        sw2.WriteLine(linedata)

        linedata = branchname & " (" & branchcode & ")"
        padlength = ((pagewidth - linedata.Length) / 2) + linedata.Length
        linedata = linedata.PadLeft(padlength, " ")
        sw2.WriteLine(linedata)

        linedata = " "
        sw2.WriteLine(linedata.PadLeft(pagewidth - 1, " "))

        linedata = "Menu Report For FUF Upload"
        padlength = ((pagewidth - linedata.Length) / 2) + linedata.Length
        linedata = linedata.PadLeft(padlength, " ")
        sw2.WriteLine(linedata)
        ...
        linedata = "-"
        sw2.WriteLine(linedata.PadLeft(pagewidth - 1, "-"))
        sw2.WriteLine("File Name               Menu Name                      Table Name            No. of Records    No. of Records uploaded")
        linedata = "-"
        sw2.WriteLine(linedata.PadLeft(pagewidth - 1, "-"))
        ...
        While dr5.Read
            ...
            If fufname <> "" Then
                ...
                If fufname = "CU3" Then
                    linecount = 5000
                ElseIf fufname = "CU7" Then
                    linecount = 5000
                ElseIf fufname = "CU8" Then
                    linecount = 5000
                ElseIf fufname = "AC1" Then
                    linecount = 6000
                ElseIf fufname = "AC2" Then
                    linecount = 3000
                ElseIf fufname = "LAM" Then
                    linecount = 1000
                ElseIf fufname = "LRS" Then
                    linecount = 1000
                Else
                    linecount = 99999999
                End If
                ...
                Dim sql12 As String
                sql12 = "SELECT COUNT(1) COUNT FROM C_MISPRINT  WHERE SOLID = '" & fufname & "'"
                Dim cmd12 As New OracleCommand(sql12, oracle_conn)
                Dim dr12 As OracleDataReader = cmd12.ExecuteReader()
                While dr12.Read
                    totalcount = dr12.Item("COUNT").ToString.Trim
                End While
                dr12.Close()
                ...
                If fufname = "CU1" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "CU2" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "CU3" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "CU5" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "CU6" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "CU7" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "RCT" Then
                    menuname = "Define Manually in RRCDM Menu"
                    tablename = ""
                ElseIf fufname = "CU8" Then
                    menuname = "UPLOAD"
                    tablename = "CMG"
                ElseIf fufname = "AC1" Then
                    menuname = "UPLOAD"
                    tablename = "GAM"
                ElseIf fufname = "AC1_NND" Then
                    menuname = "UPLOAD"
                    tablename = "GAM"
                ElseIf fufname = "AC3" Then
                    menuname = "UPLOAD"
                    tablename = "GAM"
                ElseIf fufname = "TAM_RD" Then
                    menuname = "UPLTAM"
                    tablename = ""
                ElseIf fufname = "TAM_FD" Then
                    menuname = "UPLTAM"
                    tablename = ""
                ElseIf fufname = "TAM_KVD" Then
                    menuname = "UPLTAM"
                    tablename = ""
                ElseIf fufname = "LAM" Then
                    menuname = "UPLLAM"
                ...
                ElseIf fufname = "AC2" Then
                    menuname = "UPLOAD"
                    tablename = "GAM"
                ElseIf fufname = "AC4" Then
                    menuname = "UPLOAD"
                    tablename = "GAM"
                ElseIf fufname = "ALT" Then
                    menuname = "UPLLIEN"
                    tablename = ""
                ElseIf fufname = "SRM" Then
                    menuname = "UPLCLM"
                    tablename = ""
                ElseIf fufname = "CBS" Then
                    menuname = "CHQUPLD"
                    tablename = ""
                ElseIf fufname = "SPT" Then
                    menuname = "UPLSPT"
                    tablename = ""
                ElseIf fufname = "CIU" Then
                    menuname = "UPLCI"
                    tablename = ""
                ElseIf fufname = "DHT" Then
                    menuname = "UPLDHT"
                    tablename = ""
                End If
                ...
                If totalcount > linecount Then
                    ...
                    sw = New StreamWriter(folderpath1 & "\" & branchcode & fufname & "_1.LST")
                    Dim sql3 As String
                    ...
                    While dr3.Read
                        ...
                        If (record_number = linecount) Then
                            ...
                            filename2 = branchcode & fufname & "_" & file_count - 1 & ".LST"
                            linedata = ""
                            linedata = linedata & filename2.ToString().Trim.PadRight(22).Substring(0, 22) & "  "
                            linedata = linedata & menuname.ToString().Trim.PadRight(29).Substring(0, 29) & "  "
                            linedata = linedata & tablename.ToString().Trim.PadRight(20).Substring(0, 20) & "  "
                            linedata = linedata & record_number.ToString().Trim.PadLeft(14).Substring(0, 14) & "  "
                            sw2.WriteLine(linedata)
                            sw = New StreamWriter(folderpath1 & "\" & branchcode & fufname & "_" & file_count & ".LST")
                            record_number = 0
                        End If
                    ...
                    filename3 = branchcode & fufname & "_" & file_count & ".LST"
                    linedata = ""
                    linedata = linedata & filename3.ToString().Trim.PadRight(22).Substring(0, 22) & "  "
                    linedata = linedata & menuname.ToString().Trim.PadRight(29).Substring(0, 29) & "  "
                    linedata = linedata & tablename.ToString().Trim.PadRight(20).Substring(0, 20) & "  "
                    linedata = linedata & record_number.ToString().Trim.PadLeft(14).Substring(0, 14) & "  "
                    sw2.WriteLine(linedata)

                ElseIf totalcount <= linecount Then

                    Dim reccnt As Integer = 0
                    Dim sw1 As StreamWriter
                    sw1 = New StreamWriter(folderpath1 & "\" & branchcode & fufname & ".LST")
                    Dim sql99 As String
                    sql99 = "SELECT REPORTDATA FROM C_MISPRINT  WHERE SOLID = '" & fufname & "' ORDER BY SERIALNO,SUBSERIALNO"
                    Dim cmd99 As New OracleCommand(sql99, oracle_conn)
                    Dim dr99 As OracleDataReader = cmd99.ExecuteReader()

                    While dr99.Read
                        Dim linedata2 As String
                        linedata2 = dr99(0)
                        processmessage("Generating File " & branchcode & fufname & ".LST")
                        sw1.WriteLine(linedata2)
                        reccnt = reccnt + 1
                    End While
                    dr99.Close()
                    sw1.Close()
                    Dim filename2 As String
                    filename2 = branchcode & fufname & ".LST"
                    linedata = ""
                    linedata = linedata & filename2.ToString().Trim.PadRight(22).Substring(0, 22) & "  "
                    linedata = linedata & menuname.ToString().Trim.PadRight(29).Substring(0, 29) & "  "
                    linedata = linedata & tablename.ToString().Trim.PadRight(20).Substring(0, 20) & "  "
                    linedata = linedata & reccnt.ToString().Trim.PadLeft(14).Substring(0, 14) & "  "
                    sw2.WriteLine(linedata)
                End If
        ...
        If IsNumeric(reportid.Substring(0, 1)) = True Then
            ...
            Dim startid As String = reportid.Substring(0, 2)
            Dim endid As String = reportid.Substring(2, 2)
            sql8 = "SELECT SLNO,REPORT_ID FROM FI WHERE REPORT_TYPE = 'FUF' AND SLNO BETWEEN " & startid & " AND " & endid & " ORDER BY SLNO"

        ElseIf reportid = "ALL" Then

            sql8 = "SELECT SLNO,REPORT_ID FROM FI WHERE REPORT_TYPE = 'FUF' ORDER BY SLNO"
        ...
        '' COMPREESING FILES HAVING SIZE > 1 MB
        For Each file2 As String In Directory.GetFileSystemEntries(folderpath1)
            Dim file_info As System.IO.FileInfo
            Dim file_size As Integer
            Dim file_name As String
            Dim extension As String
            file_info = My.Computer.FileSystem.GetFileInfo(file2)
            file_name = file_info.Name
            file_size = file_info.Length
            If file_name.Substring(5, 3) <> "CU3" Then
                If file_name.Contains(".") Then
                    extension = file_name.Substring(file_name.LastIndexOf("."), file_name.Length - file_name.LastIndexOf("."))
                    'MsgBox(extension)
                End If
    ...
    Private Sub option606()   'eNMGB Migration - Data Check Reports
        Dim mdate As String = InputBox("Enter migration date", "", txtdate.Text)
        ...
        MsgBox("Files generated Successfully", MsgBoxStyle.Information, "Process Completed")
    End Sub
    Private Sub option607()             'eNMGB Migration - Report of MT tables
        ...
        If username.Length > 4 Then
            ...
            If username.Substring(0, 4).ToUpper = "MIG0" Then
                ...
                processmessage("Checking prerequisites")
                Dim sql13 As String
                sql13 = "SELECT COUNT(1) COUNT FROM MIG_CGL"
                Dim cmd13 As New OracleCommand(sql13, conn)
                Dim dr13 As OracleDataReader = cmd13.ExecuteReader()
                While dr13.Read
                    tempcount = dr13.Item("COUNT").ToString.Trim
                End While
                ...
                If tempcount = 0 Then
                    MsgBox("Cannot Proceed!!! Process 606 is not completed.", MsgBoxStyle.Information, "Alert!!!")
                End If
    ...
    End Sub
    Private Sub option608()   'eNMGB Migration - Data Comparison Reports
        ...
        If username.Length > 4 Then
            ...
            If username.Substring(0, 4).ToUpper = "MIG0" Then
                ...
                processmessage("Checking prerequisites")
                Dim sql13 As String
                sql13 = "SELECT COUNT(1) COUNT FROM MIG_CITY_CODE1"
                Dim cmd13 As New OracleCommand(sql13, oracle_conn)
                Dim dr13 As OracleDataReader = cmd13.ExecuteReader()
                While dr13.Read
                    tempcount = dr13.Item("COUNT").ToString.Trim
                ...
                If tempcount = 0 Then
                    MsgBox("Cannot Proceed!!! Process 607 is not completed.", MsgBoxStyle.Information, "Alert!!!")
                End If
                ...
                Dim folderpath1 As String = "c:\du\" & branchcode & "_" & branchname & "\" & rptoption & "_" & branchcode & "_" & txtmenu.Text.ToString.Substring(18, procedure_len - 18)
    ...
    End Sub
    Sub option610()      'Check 2059 files
    ...
    Private Sub populate_label()
        ...
        ElseIf txtmenu.Text = "eNMGB Migration - Create Branch Data" Then
            ...
            lblinfo4.Text = "Create a folder d:\dump\script.Place the following files."
            ...
            lblinfo7.Text = "Create a folder D:\dump\BR_solid(In five digit eg:-BR_00001)."
            lblinfo8.Text = "Place the Bsolid.dmp,Tsolid.dmp in D:\dump\BR_solid."
            lblinfo9.Text = ""
            lblinfo10.Text = ""
        ...
        ElseIf txtmenu.Text = "Migration Tool Data Entry Status Email" Then
            ...
            lblinfo9.Text = ""
            lblinfo10.Text = ""
        ElseIf txtmenu.Text = "eNMGB Migration - Upload 2509 Files" Then
            rptoption = 609
            'lblinfo1.Text = "Database: Ten; User Name: EMail"
        ...
        ElseIf txtmenu.Text = "eNMGB Migration - Check 2509 Files" Then
            rptoption = 610
            'lblinfo1.Text = "Database: Ten; User Name: EMail"
            lblinfo2.Text = "Should execute only after uploading 2509 files through Option No - 609"
            lblinfo3.Text = ""
            lblinfo4.Text = ""
            lblinfo5.Text = ""
            lblinfo6.Text = ""
